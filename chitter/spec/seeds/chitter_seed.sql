DROP TABLE IF EXISTS peeps, users, tags;

CREATE TABLE users ("id" SERIAL PRIMARY KEY, "username" text, "password" text, "email" text);

CREATE TABLE peeps ("id" SERIAL PRIMARY KEY, "user_id" int, "time" TIMESTAMP, "content" text,
...
CREATE TABLE tags (
  "peep_id" int,
  "user_id" int,
  constraint fk_tag1 foreign key(peep_id) references peeps(id) on delete cascade,
  constraint fk_tag2 foreign key(user_id) references users(id) on delete cascade,
